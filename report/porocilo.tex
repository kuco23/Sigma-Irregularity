...
Naslednja stvar, ki pa jo bova morala preveriti je ta, da je $(\max_{G_n \in Zn} \sigma_r(G_n))_n$ 
reda $O(n^2)$. 
...
zaporedja $(\max_{G_n \in Zn} \sigma_r(G_n))_n$, si poglejmo 
naslednji račun.
$$
\sigma_r(G) = \frac{\sigma_t(G)}{\sigma(G)} 
\leq \sigma_t(G)
...
< \sum_{(u, v) \in V(G)}n^2
= n^2  \binom{n}{2} = \frac{1}{2}(n^4 - n^3) = O(n^4).
$$
...
\\\\
Konstrukcije grafov $G_n$, za katere ima zaporedje  $(\max_{G_n \in Zn} \sigma_r(G_n))_n$  stopnjo 
...
Za majhne grafe sva našla že generirane povezane neizomorfne grafe do stopnje 9.
...
bi imeli stopnjo naraščanja $O(n^3)$, a njena prisotnost znatno povišuje vrednost $\sigma$.
\\\\
...
$(\max_{G_n \in Zn} \sigma_r(G_n))_n$, za $n \in \mathbb{N}$
...
$$c = \frac{||X(n, p)||_{2}^2}{\langle a, X(n, p)\rangle}.$$
